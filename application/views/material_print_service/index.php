...
<div class="printedBg">
    <div class="container">
        <div id="bg-index" style="display: none;">
                        ...
                        <div class="m-b-10 m-t-10">
                            <label class="input"> <i class="icon-append fa fa-user"></i>
                                <input type="email" placeholder="Enter Email address" name="email" id="email"
                                       class="required" aria-required="true" value="<?= $user_email ?>">
                            </label>
                            <!--<label class="select">
        ...
        <div class="row" id="bg-step-1" style="display:block;">
...
<div class="ourTeam" id="start_with_printing" style="display:block;">
    <div class="container">
        <div class="clear30"></div>
        <nav class="prLbSteps-nav" style="display: none;">
            <ul>
        ...
        <input type="hidden" value="rectangle" id="shape">
...
<script>
    ...
    function filter_material_data(trigger, container, type) {
        ...
        var available_in = $('#available_in').val();
        var email = $('#email').val();
        var orientation = $('#' + container).find('#label_orientation').val();
        ...
        show_loader('100', '27');
        var request = $.ajax({
            url: base + 'ajax/material_printingmaterials',
            type: "POST",
            async: "false",
            dataType: "html",
            data: {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.response == 'yes') {
                            ...
                            pre_load_get_actual_products(preferences_global);
                        }
                    }
                });
    ...
    function orientation_text() {
        ...
        if(orientation != '')
        {
            $('.roll_orientation_text').show();
            $('.roll_orientation_text_value').show();
            $('.roll_orientation_text').html('<b>' + orientation + ': </b>');
            $('.roll_orientation_text_value').html(orientation_text);
        }
        else
        {
            $('.roll_orientation_text').hide();
            $('.roll_orientation_text_value').hide();
        }
    ...
    $(document).on("click", ".step-forward", function (e) {
        ...
        if (step == 3) {
            ...
            if (validation != 'false') {
                $('.step-4').removeClass('prLbSteps-step-active');
                get_actual_products(preferences_global);
            }
        } else {
            activate_steps(step);
        }
...
*/
                ...
                success: function (data) {
                    ...
                    if (!data == '') {
                        if (data.content != '') {
                            ...
                            $('#product_summary_overview_home').html(data.content);
                            var prdid = $('#cartproductid').val();
                            //$('#design_qty_'+prdid).val(parseInt(data.design));
                            // $('#cal_btn'+prdid).click();
                            $('.additional_designs_updatebtn').hide();
    ...
    $('.preferences').on('show.bs.modal', function (e) {
        ...
        if (preferences.available_in != null && (preferences.available_in == "A4" || preferences.available_in == "A3"  || available_in == 'SRA3'  || available_in == 'A5') ) {
            $("#a4_block").show();
            $("#roll_block").hide();
    ...
    function remarketting_trigger_step(step) {
        ...
        if (step < 6) {
            ...
            if (page_name != '') {
                ...
                ?>
                history.pushState(null, null, '<?=base_url()?>material-printed-labels');
            }
            ...
            success: function (data) {
                ...
                if (data.response == 'yes')
                {
                    ...
                    if(preferences.available_in == 'A4')
                    {
                        ...
                        backtomaterial_ype = "a4-sheets";

                    } else if(preferences.available_in == 'A3') {
                        $(".name_print_type").html("sheets");
                        backtomaterial_ype = "a3-sheets";
                    } else if(preferences.available_in == 'SRA3') {
                        $(".name_print_type").html("sheets");
                        backtomaterial_ype = "sra3-sheets";
                    } else if(preferences.available_in == 'A5') {
                        $(".name_print_type").html("sheets");
                        backtomaterial_ype = "a5-sheets";
                    } else  {
                        $(".name_print_type").html("rolls");
                        backtomaterial_ype = "roll-labels";
                    }
                    ...
                    if(preferences.available_in == 'A4' || preferences.available_in == 'A3' || preferences.available_in == 'SRA3' || preferences.available_in == 'A5')
                    {
                        if(preferences.digital_proccess_a4 != '')
                        {
                            apply_preferences(preferences);
                            activate_steps(2);
                        }    
                        else
                        {
                            ...
                            pre_load_activate_steps(2);
                            return;
                        }
                    }
                    else if(preferences.available_in == 'Roll')
                    {
                        if( (preferences.digital_proccess_roll != '') && (preferences.finish_roll != '') )
                        {
                            apply_preferences(preferences);
                            activate_steps(2);
                        }    
    ...
    function pre_load_apply_preferences(data) {
        ...
        if (data.selected_size == null) {
            ...
            setTimeout(function () {
                ...
                if(data.available_in == "Roll")
                {
                    ...
                    else
                    {
                        if(data.wound_roll == 'Inside')
                        {
                            $('.roll_sheets_block').find('.dropdown-toggle').html('Orientation 5 <i class="fa fa-unsorted"></i>');
            ...
            setTimeout(function () {
                ...
                pre_load_get_actual_products(data);
                //activate_steps(3);
    ...
    function pre_load_activate_steps(step) {
        step = parseInt(step);
    ...
    function pre_load_get_actual_products(allPreferences) {
        ...
        $.ajax({
            url: base + 'ajax/material_getfinalproducts',
            type: "POST",
            async: "false",
            dataType: "html",
            data: {
                ...
                ManufactureID:allPreferences.ManufactureID,
            ...
            success: function (data) {
                ...
                // activate_steps(3);
                pre_load_add_item(allPreferences);
                $('#aa_loader').hide();
            }
        });
    }
    ...
    function pre_load_add_item(allPreferences) {
        ...
        var max_qty = parseInt($('#max_qty' + id).val());
            ...
            $.ajax({
                url: base + 'ajax/material_continue_with_product',
                type: "POST",
                async: "false",
                dataType: "html",
                data: {
    ...
    // WHEN BUTTON CLICKED FUNCTION STARTS
            ...
            if (data.selected_size == null) {
                ...
                setTimeout(function () {
                    ...
                    if(data.available_in == "Roll")
                    {
                        ...
                        if(data.orientation != '')
                        {
                            $('.roll_sheets_block').find('.dropdown-toggle').html(data.orientation+' <i class="fa fa-unsorted"></i>');
                            $('#label_orientation').val(data.orientation);
                        }
                    }
    ...
    function get_actual_products(allPreferences) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                /*$('.roll_material_selection').find('span[name="productcode_text"]').html(data.rollcode);
                                ...
                                $('#label_coresize').html(data.rollcores);*/
                // activate_steps(3);
                add_item(allPreferences);
                $('#aa_loader').hide();
            }
        });
    }

    function add_item(allPreferences) {
        var id = allPreferences.ProductID;
        var menuid = allPreferences.ManufactureID;
        var rollfinish = '';
        var coresize = '';
    ...
    // WHEN BUTTON CLICKED FUNCTION ENDS
