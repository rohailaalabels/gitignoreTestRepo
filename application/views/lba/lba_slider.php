       ...
       <div class="lba-mat-slider" style="background:none !important;">
        ...
        <?  $i=1;
		   foreach($designs as $row){
			  ...
			  $shapesize = $this->db->query("SELECT COUNT(*) as count from lba_sets WHERE Designid = '".$row->designID."' and display = 1")->row();
                ...
                <div class="item  <?=$classu?>">
                 ...
                 <div class="caption3 text-center RelatedCaption">
                     ...
                     <a href="<?=base_url()?>free-label-design-templates/<?=$link1?>/<?=$link2?>/<?=$row->designID?>/" class="btn-block btn-sm btn orange" role="button" > <i class="fa fa-arrow-circle-right"></i> Select Design</a>
               </div>
                            <div class="clear"></div>
                          </div>
                        </div>
