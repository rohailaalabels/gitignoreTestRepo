<div class="mat-sep-2017">
  <?
		foreach($materials as $rec){
			if(preg_match("/Roll/",$rec->ProductBrand)){$type = 'Rolls';$iconspath = 'rolls';}
			else{$type = 'A4'; $iconspath = 'sheets';}
		...
		if(isset($colours[$last]->LabelColor) and $colours[$last]->LabelColor != $rec->LabelColor and count($colours) > 1 and empty($productid)){
			$condition = " LabelFinish LIKE '".$rec->LabelFinish."' AND ColourMaterial LIKE '".$rec->ColourMaterial."' AND LabelColor LIKE '".trim($colours[$last]->LabelColor)."' AND CategoryID LIKE '".$rec->CategoryID."' AND Adhesive LIKE '".$rec->Adhesive."'";
		...
		if(preg_match("/Roll/",$rec->ProductBrand)){
			...
			//$type_url = 'labels-by-adhesive-rolls';
			...
			$mat_image = Assets.'images/material_images/sheets/'.$material_code.'.png';
				...
				//$type_url = 'labels-by-adhesive-sheets';
		...
		$group_name = (isset($materialinfo['group_name']) and $materialinfo['group_name']!='')?$materialinfo['group_name']:'';
			...
			//$material_url = strtolower(str_replace(" ","-", trim($info['material_name'])));
			$material_url = strtolower(str_replace(" ","-", trim($rec->Material1)));
  ...
  <section data-mat-filters="<?=$rec->ColourMaterial?>" data-reset="reset" >
    <div class="row productdetails <?=($type=='Rolls')?'roll-products':''?>" data-value="<?=$rec->ProductID?>" 
      ...
      <article class="col-lg-12 col-md-12 col-sm-12 col-xs-12 mat-detail">
        ...
        <div class="clear"></div>
        <div class="row specs">
          <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2"> <img onerror='imgError(this);' class="img-responsive product_material_image" src="<?=$mat_image?>" alt="<?=$rec->Material1?>"> </div>
          <div class="col-lg-10 col-md-10 col-sm-10 col-xs-10">
            ...
            <div class="row">
              <div class="col-lg-5 col-md-5 col-sm-5 col-xs-12">
                ...
                <div class="row">
                  <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6"> <a href="#" id="<?=$rec->ProductID?>" class="technical_specs" data-target=".material" data-toggle="modal" data-original-title="Tecnial Specification"> <i class="fa fa-info-circle"></i> Material Specification</a> </div>
                  <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6"> <a href="#" id="<?=$group_name?>" class="applications" data-target=".lb_applications" data-toggle="modal" data-original-title="Tecnial Specification"> <i class="fa fa-table" aria-hidden="true"></i> Label Applications</a> </div>
                </div>
                <div class="row">
                  ...
                  <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <ul class="clr-thumbs colourpicker">
                      <? foreach($colours as $row){
							...
							$colour_icon = Assets.'images/material_images/colours/'.$iconspath.'/'.$row->imagecode.'.png';
							if (getimagesize($colour_icon) == false) {
								$colour_icon = Assets.'images/material_images/colours/'.$iconspath.'/white.png';
							}
						   ?>
              ...
              <div class="col-lg-7 col-md-7 col-sm-7 col-xs-12 m-t-10">
				...
				if(preg_match("/Roll/",$rec->ProductBrand))
				{
					...
					$inkjet_div = "vertical-align:bottom";
				}
				else
				{
					$thermal_div = $d_thermal_div = 'visibility:hidden';
				}
				?>
                ...
                <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 labels-form">
                  <label class="select no-margin">
                    ...
                    <i></i> </label>
                  <a href="<?=$url?>" class="btn btn-block orangeBg pull-right product-url" >Select Label Shape &amp; Size <i class="fa fa-arrow-circle-right"></i></a> </div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="row">
          <div class="col-md-5" style="margin-top:5px;">
            <table class="table printer" border="0" style="border:none;">
              <tbody>
                <tr>
                  <td align="left" valign="center" style="font-size:12px; border:none;vertical-align: middle;width:25%;"><small style="margin-top:10px;font-size:12px;">Printer<br />
                    Compatibility</small></td>
                  <td  align="left" style="vertical-align: middle;font-size:9px; border:none; width:25%;<?=$laser_div?>"> Laser <a data-toggle="tooltip-product" data-placement="top" class="laser_printer_div" title="" data-original-title="<?=$comp['laser_text']?>" href="javascript:void(0);"><i class="fa fa-info-circle"></i></a><br /><br />
                    <img onerror='imgError(this);' class="laser_printer_img" width="50" src="<?=Assets?>images/<?=$comp['laser_img']?>"  /></td>
                  <td  align="left" style="vertical-align: middle; font-size:9px;border:none;width:25%;<?=$inkjet_div?>"> Inkjet <a data-toggle="tooltip-product" data-placement="top" title="" class="inkjet_printer_div" data-original-title="<?=$comp['inkjet_text']?>" href="javascript:void(0);"><i class="fa fa-info-circle"></i></a><br /><br />
                    <img onerror='imgError(this);' class="inkjet_printer_img" width="50" src="<?=Assets?>images/<?=$comp['inkjet_img']?>"  /></td>
                  <td  align="left" style="vertical-align: middle; font-size:9px;border:none;width:25%;<?=$d_thermal_div?>"><?php if(preg_match("/Roll/",$rec->ProductBrand)):?>
                    Direct<br />
                    ...
                    <img onerror='imgError(this);' class="direct_printer_img" width="50" src="<?=Assets?>images/<?=$comp['d_thermal_img']?>"  />
                    <?php else:?>
                    <?php endif;?></td>
                  <td align="left" style="vertical-align: middle; font-size:9px;border:none;width:25%; <?=$thermal_div?>"><?php if(preg_match("/Roll/",$rec->ProductBrand)):?>
                    Thermal<br />
                    ...
                    <img onerror='imgError(this);' class="thermal_printer_img" width="50" src="<?=Assets?>images/<?=$comp['thermal_img']?>"  />
                    <?php else:?>
                    <?php endif;?></td>
                </tr>
              </tbody>
