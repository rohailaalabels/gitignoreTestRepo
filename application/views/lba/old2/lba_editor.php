  ...
  <div class="row">
    <div class="col-xs-12  col-sm-6 col-md-8 ">
      <ol class="breadcrumb  m0">
        ...
        <li class="active">Labels by application / <a href="<?=base_url()?>lba-page/<?=$category->link?>">
          <?=$category->name?>
          </a> / <a href="<?=base_url()?>lba-page/<?=$category->link?>/<?=strtolower(str_replace(" ","-",$category->sub_name))?>">
          <?=$category->sub_name?>
          </a> /
          <?=$category->CID?>
        </li>
  ...
  <div class="modal-dialog modal-lg" style="width:82%">
    <div class="modal-content no-padding">
      ...
      <div class="panel-heading">
        ...
        <div class="panel-body">
           <? include(APPPATH.'/views/barcode/main.php');?>
           <style>
		   .prLbSteps-nav {
	/* background: #fff; */
...
$(document).on("click", ".calculate_printed", function(e) {
	...
	if(available_in=="both"){
	 $('.add_to_cart').css('margin-bottom','16px');
	}else{
	 $('.add_to_cart').css('margin-bottom','31px');
	}
 ...
 $('.lb_applications').modal('hide');	
  ...
  if(document.getElementById("Source")!=null){
      swal({
		...
		function(isConfirm) {
		    if(isConfirm) {
			  load_panel_lba(label_id,is_user_design_selected);	
		    }else{ return false;	}
	     });
   ...
   function load_panel_lba(label_id,is_user_design_selected){
	   $.ajax({
			url: base+'home/load_flash_panel',
			type:"POST",
			async:"false",
