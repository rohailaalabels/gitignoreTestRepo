                ...
                <div class="" >
                  ...
                  <div class="table-responsive col-md-12 p0 border0">
                    <table class="table table-bordered table-hover ">
                      <thead class="">
                        <tr>
                          <th style="width:10%;">Code/Templates</th>
					...
					if($product->Printing == "Y")
					{
						...
						{
							$sql = $this->home_model->get_sum_of_designed_artworks($rec->OrderNumber, $product->ProductID);
					...
					else
					{
                     ...
                     <tr>
                       <td style="text-align:center;"><?=$details['ManufactureID']?><div class="template-btns">
                        <?php
						if($product_type == "Sheets")
						{
							...
							{
							?>
								<a rel="nofollow" data-toggle="tooltip" data-placement="right" title="" href="<?=base_url()?>download/pdf/<?=$pdfFile?>" class="btn orangeBg" role="button" data-original-title="Download PDF Template"> <i class="fa fa-file-pdf-o f-28"></i> </a>
								<?php
							}
							if(isset($wordFile) and $wordFile != '')
							{
							?>
								<a data-toggle="tooltip" title="" data-placement="right" rel="nofollw" href="<?=Assets?>images/office/word/<?=$wordFile?>" class="btn blueBg" role="button" data-original-title="Download Word Template"> <i class="fa f-28 fa-file-word-o "></i> </a>
						<?php
							}
						}
						?>
					...
					?></td>
                        <td style="text-align:center;"><div class="labels-form ">
							...
							if(preg_match("/Roll Labels/i",$details['ProductBrand']) && $product->Printing =="Y"){
								...
								if($no_of_labels == 0)
								{
									$no_of_labels = $details['LabelsPerSheet']*$product->Quantity;
								}
								echo $no_of_labels. " Labels" . "<br>(" . $product->Quantity . " Rolls) <br>"; if($product->Print_Qty > 0){echo $product->Print_Qty . ' ' . $designs;}
