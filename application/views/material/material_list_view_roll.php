...
foreach ($materials as $rec) {
    ...
    $materialinfo = $this->db->query("Select tooltip_info,short_name,group_name,filter_group,filter_color from material_tooltip_info WHERE material_code LIKE '" . $material_code . "' and type LIKE '%$type%'")->row_array();
    ...
    $adhesive_condition = "type LIKE '" . $type . "' AND filter_group LIKE '" . $materialinfo['filter_group'] . "'";
    ...
    if (isset($colours[$last]->LabelColor) and $colours[$last]->LabelColor != $rec->LabelColor and count($colours) > 1 and empty($productid)) {
        ...
        $result = $this->db->query("Select ProductID,LabelsPerSheet,ManufactureID,SpecText7,LabelFinish,ColourMaterial,LabelColor,Adhesive,
			ProductCategoryName, Printable
    ...
    //$materialinfo = $this->db->query("Select tooltip_info,short_name,group_name from material_tooltip_info WHERE material_code LIKE '".$material_code."'")->row_array();
    ...
    if ($material_discount) {
        $promotion = ' <br /> <strong style="color:#fd4913"> Special Offer Save ' . $material_discount . '% While Stocks Last </strong> ';
    }

    if (preg_match("/PEOP/i", $rec->ManufactureID) || preg_match("/PEWP/i", $rec->ManufactureID)) {
        $promotion = ' <br /> <strong style="color:#fd4913">NEW MATERIAL</strong> replacing PETP on rolls, please request a sample if you have ordered previously. ';
    }
    ...
    <section data-mat-filters="<?= $materialinfo['filter_group'] ?>" data-reset="reset">
        <div class="row productdetails roll-products" data-value="<?= $rec->ProductID ?>"
             data-finish="<?= $rec->LabelFinish ?>" data-material="<?= $rec->ColourMaterial ?>">
            <div class="hiddenfields">
                <input type="text" value="" class="cart_id"/>
                <input type="text" value="<?= $rec->ProductID ?>" class="product_id"/>
                <input type="text" value="<?= $rec->ManufactureID ?>" class="manfactureid"/>
                <input type="text" value="<?= $rec->LabelsPerSheet ?>" class="LabelsPerSheet"/>
                <input type="text" value="" class="digitalprintoption"/>
                <input type="text" value="<?= $min_labels_per_roll ?>" class="minimum_printed_labels"/>
                <input type="text" value="<?= $max_labels ?>" class="maximum_printed_labels"/>
                <input type="text" value="<?= $min_qty ?>" class="minimum_quantities"/>
                <input type="text" value="<?= $max_qty ?>" class="maximum_quantities"/>
                <input type="text" value="0" data-qty="0" data-presproof="0" data-rolls="0"
                       id="uploadedartworks_<?= $rec->ProductID ?>"/>
                <input type="text" value="<?= $rec->Printable ?>" class="PrintableProduct"/>
                <input type="text" value="1" class="orientation"/>
                <input type="text" value="<?= $rec->ProductCategoryName ?>" class="category_description"/>
            </div>
        ...
        <?= $rec->Material1 ?>
                ...
                <div class="clear"></div>
                <div class="row specs">
                    <div class="col-lg-3 col-md-3 col-sm-2 col-xs-3"><img onerror='imgError(this);'
                        ...
                        <ul class="clr-thumbs colourpicker">
                            ...
                            foreach ($colours as $row) {
                                ...
                                if ($details['Shape_upd'] == "Rectangle") {
                                    $colour_icon = Assets . 'images/material_images/colours/rolls/rectangle/' . $row->imagecode . '.png';
                                } else {
                                ...
                                <li class="<?= $active_colour ?> data-reset-colour" data-toggle="tooltip"
                                    ...
                                    data-colour-filters="<?= $row->LabelColor ?>"><a class="mat_<?= $colourclass ?>"
                                                ...
                                                src="<?= $colour_icon ?>" alt="<?= $row->colour_name ?>"></a></li>
                            <? } ?>
                        </ul>
                    </div>
                </div>
                            ...
                            <tr>
                                <td align="left" valign="center"
                                    style="font-size:12px; border:none;vertical-align: middle;width:25%"><small
                                            style="margin-top:10px;font-size:12px;">Printer Compatibility</small></td>
            ...
            <article class="col-lg-7 col-md-7 col-sm-12 col-xs-12 mat-tabs">
                <ul class="nav nav-justified" role="tablist">
                    <li class="col-xs-4 no-padding active plainoption"><a href="#tab_plain<?= $rec->ProductID ?>"
                                                                          aria-controls="" role="tab" data-toggle="tab">Plain
                            Labels</a></li>
                    <li class="col-xs-4 no-padding printedoption <?= ($rec->Printable == 'N') ? 'hide' : '' ?>"><a
                                href="#tab_printed<?= $rec->ProductID ?>" aria-controls="" role="tab" data-toggle="tab">Printed
                            Labels</a></li>
                    <li class="col-xs-4 no-padding"><a href="#tab_sample<?= $rec->ProductID ?>" aria-controls=""
                                                       role="tab" data-toggle="tab">Material Sample</a></li>
                </ul>
                <div class="tab-content">
                    <div id="tab_plain<?= $rec->ProductID ?>" class="tab-pane tabplain active" role="tabpanel">
                        <div class="row p-t-10" style="margin-top:20px;">
                            <div class="col-xs-12 ofq hidden-sm hidden-xs">
                                <div class="col-xs-4 main-box">
                                    <div class="row">
                                        <div class="col-xs-2 no-padding-right"><img onerror='imgError(this);'
                                ...
                                <div class="col-xs-4 main-box">
                                    <div class="row">
                                        <div class="col-xs-2 no-padding-right"><i class="fa fa-truck l-icon"
                                ...
                                <div class="col-xs-4 main-box">
                                    <div class="row">
                                        <div class="col-xs-2 no-padding-right"><i class="fa fa-check-square-o l-icon"
                                                                                  aria-hidden="true"></i></div>
                                        <div class="col-xs-10  no-padding-right"><b>QUALITY ASSURANCE GUARANTEE</b>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="row">
                                    <div class="col-lg-4 col-md-4 col-sm-4 col-xs-6">
                                        <p><strong>Labels Per Roll</strong>
                                            <input maxlength="4" type="text" placeholder="100+"
                                                   class="form-control allownumeric plainlabels">
                                    ...
                                    <div class="col-lg-4 col-md-4 col-sm-4 col-xs-6">
                                        <p><strong style="font-size:12px;">Number of Rolls</strong>
                                            <input maxlength="3" type="text" placeholder="<?= $min_qty ?>+"
                                                   class="form-control allownumeric plainrolls">
                                            <small>Multiples of
                                                <?= $min_qty ?>
                                                only</small></p>
                                    </div>
                            ...
                            <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12 quotation">
                                ...
                                <a href="#" class="price_breaks pull-right text-right"
                                   data-target=".pbreaks" data-toggle="modal" data-original-title="Volume Price Breaks">View
                                    volume price breaks</a></div>
                            <div class="clear"></div>
                            <div class="service">
                                <div class="addprintingoption hide pull-right"><a
                                            href="#tab_printed<?= $rec->ProductID ?>" aria-controls="" role="tab"
                                            data-toggle="tab" class="apf printpriceoffer"> <i
                                                class="fa fa-hand-o-right pull-left" aria-hidden="true"></i> <span>Add printing for only <b
                ...
                16.59</b>?</span> </a></div>
                    ...
                    <div id="tab_printed<?= $rec->ProductID ?>" class="tab-pane tabprinted" role="tabpanel">
                        <div class="row" style="margin-top:70px">
                            <div class="col-lg-7 col-md-7 col-sm-7 col-xs-12">
                                    ...
                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center qty">
                                        <div class="row">
                                            <div class="col-lg-4 col-md-4 col-sm-6 col-xs-6">
                                                <input maxlength="7" type="text" placeholder="Enter Labels"
                                                       class="form-control allownumeric printlabels">
                                                ...
                                                <div class="dm-box">
                                                    <div class="btn-group btn-block dm-selector digital_proccess-d-down">
                                                        ...
                                                        <ul class="dropdown-menu btn-block">
                                                            <li><a data-toggle="tooltip-digital" data-value=""
                                                            ...
                                                            foreach ($a4_prints as $row) {
                                                                ...
                                                                <li style=" <?= $white_digital_style ?>"><a
                                                                            data-toggle="tooltip-digital"
                                                                            data-trigger="hover" data-placement="right"
                                                                            title="<?= $row->tooltip_info ?>"
                                                                            data-id="digital"
                                                                            data-value="<?= $row->name ?>">
                                                                        <?= $row->name ?>
                                                                    </a></li>
                                    ...
                                    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6 labels-form">
                                        <label class="select">
                                            <select name="finish" class="labelfinish">
                                                ...
                                                <?php
                                                if (preg_match("/PETP/i", $rec->ManufactureID) || preg_match("/PETC/i", $rec->ManufactureID) || preg_match("/PETH/i", $rec->ManufactureID)) {
                                                ...
                                                <option value="Gloss Lamination" <?= $disabled ?>>Gloss Lamination
                                                </option>
                                                <option value="Matt Lamination" <?= $disabled ?> >Matt Lamination
                                                </option>
                                ...
                                <div class="clear"></div>
                                <div class="row">
                                    <div class="col-lg-10 col-md-10 col-sm-7 col-xs-12"><a href="#"
                                                                                           class="btn btn-primary art-btn btn-block artwork_uploader"
                                                                                           data-target=".artworkModal1"
                                                                                           data-toggle="modal"
                                                                                           data-original-title="Upload Your Artwork">
                                            <i class="fa fa-cloud-upload" aria-hidden="true"></i>&nbsp; Click here to
                                            Upload Your Artwork</a></div>
                                </div>
                            ...
                            <div class="col-lg-5 col-md-5 col-sm-5 col-xs-12 quotation">
                                ...
                                <div class="price_promise_div row">
                                    <div class="col-md-10 pull-right">
                                        <h5><a data-toggle="tooltip-product" data-placement="top" class="" title=""
                                               data-original-title="If within 7 days of buying this printed roll label product from us, you find the same product cheaper to purchase online at any other UK label website, we will refund 100% of the difference in the printing price."
                                ...
                                <div class="printedprice_box" style="display:none;">
                                    <table class="price" width="100%" cellspacing="0" cellpadding="0" border="0">
                                        ...
                                        <tr class="halfprintprice" style="">
                                            <td style=" width:80%;" class="text-left phead color-orange">Half Price
                                                Promotion
                                            </td>
                                        ...
                                        <tr class="labelfinishprice" style="">
                                            ...
                                            <td style=" width:20%;" class="labelfinishtext color-orange text-right">
                                                0.00
                                            </td>
                                        </tr>
                                ...
                                <?php if (preg_match('/WTDP/', $rec->ManufactureID)): ?>
                                    <button style="display:none;" class="btn orangeBg pull-right get_a_quote_printed"
                                            type="button" data-toggle="modal" data-target="#get_a_quote"
                                            data-printing="Y"> Get a Quote <i class="fa fa-calculator"></i></button>
                                <?php else: ?>
                                    <button style="display:none;" class="btn orangeBg pull-right add_printed"
                                            type="button"> Add to basket <i class="fa fa-calculator"></i></button>
                                <?php endif; ?>
                                ...
                                <?php if (preg_match('/WTDP/', $rec->ManufactureID)): ?>
                                    <p class="text-right"
                                       style="font-size: 12px !important;display: block;clear: both;">Contact Customer
                                        Care for Product <br/>Availability in this Quantity</p>
                                <?php endif; ?>
                    ...
                    <div id="tab_sample<?= $rec->ProductID ?>" class="tab-pane" role="tabpanel">
                        <div class="col-xs-12"><small class="sample"> All material samples are supplied on a strip of
                                roll labels for the purpose of assisting the choice of face-stock colour and appearance.
                                Along with assessing the print quality and application suitability of the
                                adhesive.</small>
                            <div class="clear10"></div>
                            <div class="row">
                                <div class="col-lg-6 col-md-6 col-sm-7 col-xs-12"><small class="note sample">Please
                                        note: The material sample supplied will not be the shape and size of the label/s
                                        shown on this page.</small>
