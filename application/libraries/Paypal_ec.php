	...
	*/
	function set_ec($ec_data = array(), $custom_data = false) {
		...
		//	$nvpstr.= "&SOLUTIONTYPE=Sole";
			...
			if (isset($ec_data['jobid'])) {
				$nvpstr.= "&PAYMENTREQUEST_0_CUSTOM=" . urlencode($ec_data['jobid']);
			}
			if (isset($ec_data['currency'])) {
				$nvpstr.= "&PAYMENTREQUEST_0_CURRENCYCODE=" . urlencode($ec_data['currency']);
			}
			if (isset($ec_data['email'])) {
				$nvpstr.= "&EMAIL=" . urlencode($ec_data['email']);
			}
			if (isset($ec_data['desc'])) {
				$nvpstr.= "&PAYMENTREQUEST_0_DESC=" . urlencode($ec_data['desc']);
			}
