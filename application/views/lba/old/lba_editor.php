...
function update_header_login(username){
 ...
 function update_lba_setpanel(){
  var designid = <?=$this->uri->segment(2)?>;
  var setid = <?=$this->uri->segment(3)?>;
  var labelid = $(".lba-nav-tabs .active a").attr('data-labelid');
     $.ajax({
		url: base+'home/lba_sets_data',
		type:"POST",
		async:"false",
		dataType: "html",
		data:{designid:designid,setid:setid},
		success: function(data){
		  if(data){
			$('#lba_sets_data').html(data); 
			$('[data-labelid='+labelid+']').trigger('click'); 
		  }
	   }  
	});
 }	
